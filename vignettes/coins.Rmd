...
This vignette gives a guide to building "coins", which are the object class used throughout COINr, and "purses", which are time-index collections of coins.
...
The contents of a coin will be explained in more detail in the following sections. For now, let's see how to build a coin.

# Building coins

To build a coin you need to use the `new_coin()` function. The main two input arguments of this function are two data frames: `iData` (the indicator data), and `iMeta` (the indicator metadata).
...
To see an example of what `iData` looks like, we can look at the built in ASEM data set:
...
The `iMeta` data frame essentially gives details about each of the columns found in `iData`, as well as details about additional data columns eventually created by aggregating indicators. This means that the
entries in `iMeta` must include *all* columns in `iData`, *except* the three "special" column names: `uCode`, `uName`, and `Time`. In other words, all column names of `iData` should appear in `iMeta$iCode`, except the three special cases mentioned.
...
These are the aggregates that will be created by aggregating indicators. These will only be created when we call the `aggregate()` function (see relevant vignette). We also have groups:
...
Apart from the `level_names` argument, `new_coin()` also gives the possibility to only pass forward a subset of the indicators in `iMeta`. This is done using the `exclude` argument, which is useful when testing alternative sets of indicators - see vignette on adjustments and comparisons *TO ADD*.
...
If you have panel data, i.e. multiple observations for each unit-indicator pair, indexed by time, then `new_coin()` allows you to create multiple coins in one go. Coins are collected into a single object called a "purse", and many COINr functions work on purses directly. Some more information on working with panel data in this way can be found in [Panel data] *TO ADD*.
