...
If you have panel data, i.e. multiple observations for each unit-indicator pair, indexed by time, then `new_coin()` allows you to create multiple coins in one go. Coins are collected into a single object called a "*purse*", and many COINr functions work on purses directly. Some more information on working with panel data in this way can be found in [Working with Panel Data](panel_data.html).
